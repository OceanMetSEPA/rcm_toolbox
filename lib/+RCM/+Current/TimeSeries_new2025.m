classdef TimeSeries_new2025 < RCM.TimeSeries.TotalTide ... % Abstract classes first
        & RCM.Current.TimeSeries.Plotter_new2025 ...
        & RCM.TimeSeries.Base
    ...
    properties (SetAccess = public, GetAccess = public)
        ...
        Pressure double = []
        %        HeightAboveBed = NaN;
        HeightAboveBed double = NaN;
        SiteName string = "";
        CARLicence string = "CAR/L/";
        BinType string = ""; %{mustBeMember(BinType,["Bottom","Middle","Surface"])}=
        BinNo double = NaN;
        WaterDepth double = NaN;
        InstrumentModel string = "";
        InstrumentSerial double = NaN;
        DataType string = "";
    ...
    methods(Static = true)
        ...
        function TS = create(time, speed, direction, varargin)
            TS = RCM.Current.TimeSeries_new2025;
        ...
        function TS = fromHGAnalysisXls(path, varargin)
            ...
            allData = readtable(data.Filename,'sheet', data.Sheet, 'readvariablenames', 0, 'format','auto');
            [l,w] = size(allData);
            settingsTable=allData(1:14,1:2);
            settingsTable.Properties.VariableNames={'Property','Value'};
            % Convert column 2 to numeric:
            nums=cellfun(@str2double,settingsTable{:,2});
            % Strings will turn into nans!
            k=~isnan(nums);
            % Update rows that aren't nans
            % NB we have to store them as a cell in a table
            settingsTable{k,2}=num2cell(nums(k));

            data.SiteName = settingsTable{1,2};
            data.CARlicence = settingsTable{2,2};
            data.Easting  = cell2mat(settingsTable{3,2});
            data.Northing = cell2mat(settingsTable{4,2});
            data.Lat = cell2mat(settingsTable{5,2});
            data.Long = cell2mat(settingsTable{6,2});
            data.DataType = settingsTable{7,2};
            data.BinType = settingsTable{8,2};
            data.BinNo = cell2mat(settingsTable{9,2});
            data.BinHeight = cell2mat(settingsTable{10,2});
            data.WaterDepth = cell2mat(settingsTable{11,2});
            data.InstrumentModel = settingsTable{12,2};
            data.InstrumentSerial = cell2mat(settingsTable{13,2});
            data.DeploymentType = settingsTable{14,2};
            ...
            dateColumn=4;
            dateformat='dd/mm/yyyy HH:MM:SS'; % NB MM and mm have only swapped for datetime. datenum remains mm for month and MM for mins
            ...
            rawSpeedData=table2cell(rawSpeedTable);

            % remove empty rows (intended to hit empty trailing cells)
            charRows = cell2mat(cellfun(@ischar, table2cell(dt), 'UniformOutput', 0));
            dt(~charRows, :) = [];

            % remove desired trailing rows
            if ~isempty(endRow) & endRow < length(rawSpeedData)
                dt = dt(1:endRow, :);
                rawSpeedData = rawSpeedData(1:endRow, :);
            end

            % textData = table2cell(dateTable);
            textData = table2cell(dt);
            numData = cellfun(@str2double,numberData);

            % Import Date & Time Surface
            for i=1:length(textData);
                if length(textData{i,1})==10;
                    tempString=strcat(textData{i,1},' 00:00:00');
                    data.DateTime(i)=datenum(tempString, dateformat);
                else
                    data.DateTime(i)=datenum(textData{i,1}, dateformat);
                end
            end
            ...
            TS.HeightAboveBed = data.BinHeight;
            TS.SiteName = data.SiteName;
            TS.CARLicence = data.CARlicence;
            TS.BinType = data.BinType;
            TS.BinNo = data.BinNo;
            TS.WaterDepth = data.WaterDepth;
            TS.InstrumentModel = data.InstrumentModel;
            TS.InstrumentSerial = data.InstrumentSerial;
            TS.DataType     = data.DataType;
            TS.DeploymentType = data.DeploymentType
        ...
        function ts = fromStruct(s, varargin)
            ts = RCM.Current.TimeSeries_new2025.create(s.Time, s.Speed, s.Direction);
